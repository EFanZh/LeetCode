...
impl Solution {
    ...
    fn helper_layer(bottom: &[u8], blocks: &mut [u8], allowed: &[u8; MAX_PAIRS]) -> bool {
        if bottom.len() == 2 {
            allowed[usize::from((bottom[0] << SHIFT_BITS) | bottom[1])] != 0
        } else {
            Self::helper_cell(bottom, blocks, 0, allowed, &mut |next_bottom, next_blocks| {
                Self::helper_layer(next_bottom, next_blocks, allowed)
            })
        }
    }
    ...
    pub fn pyramid_transition(bottom: String, allowed: Vec<String>) -> bool {
        ...
        Self::helper_layer(&bottom, &mut blocks, &allowed_map)
    }
}
